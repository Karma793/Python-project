...
toad trout turkey turtle weasel whale wolf wombat zebra'''.split()
    ...
    infStr="_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_"
    ...
    def askandevalv(self,word,result,missed):
        guess=input()
    ...
    def start(self):
        ...
        while i < len(self.pics)-1:
            ...
            guess,right=self.askandevalv(word,result,missed)
            if guess == None:
                print("You've already entered this character.")
                continue
            print(''.join(result))
            if result==word:
                self.info("Congratulation ! You\'ve just saved a life!")
                success=True
                break
            if not right:
                missed.append(guess)
